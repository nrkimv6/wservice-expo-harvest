...
|--------|-------------|-----------|--------------|------|
| **2.5 worktree closeout residue read-back** | plan 헤더에 `> branch:` 또는 `> worktree:` 필드 존재 | `auto-done.ps1 -Json`의 `worktree_closeout_residue` 판정이 `clean` 또는 `physical_only_empty`이고, `physical_only_empty`면 tracking handoff 결과가 기록됨 | `branch_present`/`worktree_list_present`은 `WORKTREE_RESIDUE_BLOCKED`, `physical_nonempty`는 `WORKTREE_PHYSICAL_NONEMPTY_BLOCKED`, out-of-scope path는 blocker로 archive 전 중단. empty physical-only residue는 tracking 등록 또는 tracking failure warning을 남긴 뒤 archive 허용 | `/done`은 cleanup 실행 owner가 아니다. `git worktree remove`/branch delete를 재시도하지 않고 read-back guard와 handoff만 수행한다. |
