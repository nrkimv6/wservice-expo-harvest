...
> 이 표가 `.claude` merge-test surface의 blocker code 정의 source of truth다. 절차 섹션은 이 표를 참조하고, 새 blocker code를 절차 텍스트에만 추가하지 않는다.

| code | source key/evidence | severity | cleanup 영향 | recovery/next owner |
|---|---|---|---|---|
| `WORKTREE_DIRTY_BEFORE_REMOVE` | `git -C {worktree} status --porcelain` | hard blocker | 해당 worktree remove/branch delete 중단 | owner worktree에서 변경 보존/커밋/정리 후 재시도 |
| `UNTRACKED_ORIGIN_BLOB_RESIDUE_BLOCKED` | `merge-test-cleanup.ps1 -Json`: `hard_blockers`, `residue_blocker_code` | hard blocker | done/archive 및 cleanup 중단 | affected paths 표, commit/quarantine/owner plan 중 하나 선택 |
...
Rules:
- Cleanup blocker classification must read `merge-test-cleanup.ps1 -Json` first. `ignored_dirty` remains warning/evidence when `cleanup_ready=true`, `mutation_ready=true`, and `hard_blockers=[]`.
...
T4/T5 command가 `0 collected`, `0 selected`, `not found`, explicit selector mismatch, connection refused, timeout으로 실패했다가 corrected rerun 또는 recovery 후 통과하면 첫 실패 row를 삭제하지 않는다. T4/T5 evidence table에는 `result=failed -> recovered` row를 남기고, 별도 `Recovered validation ledger`에 `stage`, `original_command`, `failure`, `recovery_action`, `recovered_command`, `result`, `failure_class`, `blocks_archive`, `blocks_other_targets`, `evidence`를 기록한다. `failure_class=test_fixture_stale` 또는 `environment_failure`이고 `blocks_archive=false`, `blocks_other_targets=false`이면 archive blocker가 아니라 warning-success path다.
T5 selector drift는 deterministic하게 분류한다. 계획서의 file/class selector가 없으면 원래 selector 실패(`0 collected`/`not found`)를 `original_command`에 보존하고, same keyword grep 또는 `common/tools/plan-runner/core/validation_recovery.py` candidate read-back 결과를 `evidence`에 남긴 뒤 corrected selector만 재실행한다. TestClient 기반 `T5-http`와 live localhost 기반 `T5-http_live` evidence type은 서로 대체하지 않는다.
post-merge live read-back timeout은 곧바로 제품 회귀로 단정하지 않는다. 현재 main commit 반영 여부를 가리키는 runtime fingerprint/readiness evidence(`merge-test-preflight.ps1`의 `live_runtime_readiness`, liveness status, restart-api 로그)를 확인하고 stale runtime이면 service restart 후 같은 endpoint를 재시도한다. timeout 후 restart/read-back success는 `failure_class=environment_failure`, `result=failed -> recovered`로 기록한다.
Browser/Playwright smoke가 profile lock, missing module, MCP unavailable로 실패하면 raw Chromium CDP smoke fallback을 허용한다. 이때 original Browser/Playwright failure, CDP target, DOM/readiness evidence를 `Recovered validation ledger`에 남긴다. CDP fallback도 실패하면 `environment_failure` blocker로 남기고 성공으로 표시하지 않는다.
...
> Blocker code 정의·우선순위는 `## Blocker Policy SSOT`를 따른다.
...
6. **T5 실행** (HTTP 통합):
   ...
   **dev-runner smoke residual 계약**:
   ...
   `http_live` endpoint timeout 또는 stale success 의심 시 `live_runtime_readiness.runtime_fingerprint`, liveness status, restart-api 로그 순서로 read-back하고 필요하면 service restart 후 재시도한다. 재시도 성공은 원래 timeout을 지우지 않고 `failed -> recovered`로 남긴다.
...
- T4/T5 evidence table schema(machine-readable): target별로 `stage`, `command`, `cwd`, `result`, `exit_code`, `log_ref`, `blocker_code` 컬럼을 가진 row를 남긴다. `result` 값은 `완료`/`미실행`/`해당 없음`/`실패`/`failed -> recovered`만 허용한다. `stage` 최소 row는 `T4`, `T5-http`, `T5-http_live`이며 해당 없음은 `> T4 E2E 해당 없음:` 또는 `> T5 HTTP 해당 없음:` 블록쿼트와 non-empty `blocker_code`가 함께 있을 때만 완료 근거로 인정한다.
- `failed -> recovered` row는 원래 실패와 recovered rerun 근거가 `Recovered validation ledger`에 있고 `failure_class=test_fixture_stale|environment_failure`, `blocks_archive=false`, `blocks_other_targets=false`일 때만 warning-success로 인정한다. corrected rerun 성공만 남기고 원래 실패를 삭제하면 evidence missing으로 취급한다.
...
> Cleanup blocker code 정의·우선순위는 `## Blocker Policy SSOT`를 따른다.
...
- `result=failed -> recovered`: recovered ledger read-back이 있고 `blocks_archive=false`이면 archive를 차단하지 않는다. 단, failure_class와 원래 실패/재시도 명령이 빠지면 `t4_t5_evidence_missing`으로 보류한다.
