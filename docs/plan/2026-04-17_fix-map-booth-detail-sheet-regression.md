...
> 상태: 검토대기
> 진행률: 6/8 (75%)
> 요약: 2026-04-17 15:30 이전에는 맵에서 부스를 한 번 탭하면 상세 모달이 바로 열렸다. `17:46` 변경에서 맵 탭 계약이 "첫 탭은 선택, 같은 부스를 다시 탭하면 상세 시트 오픈"으로 바뀌었고, 이후 지도 제스처/탭 처리 강화가 겹치면서 현재는 흰 시스템 하이라이트만 보이거나 두 번째 탭이 불안정한 상태가 되었다.
...
- 오후 11시경: 맵에서 부스를 눌렀을 때 흰 테두리처럼 보이는 기본 탭 하이라이트가 먼저 보이고, 두 번째 선택도 기대한 만큼 안정적으로 동작하지 않음
...
- [`src/lib/components/ExhibitionMap.svelte`](D:/work/project/service/wtools/expo-harvest/src/lib/components/ExhibitionMap.svelte): `gestureIntent`가 있는 경우 클릭을 한 번 삼키는 구조라, "두 번 탭해야 상세" 계약과 결합되면 모바일에서 체감 실패율이 커진다.
...
3. - [ ] **모바일 실기기에서 상세 시트/제스처 공존을 확인한다** — 탭, 드래그, 핀치가 충돌하지 않는지 최종 점검
   - [ ] `src/routes/+page.svelte`, `src/lib/components/ExhibitionMap.svelte`: overview/구역 지도 각각에서 첫 탭 즉시 상세 시트가 열리고, 드래그/핀치 직후의 의도치 않은 시트 오픈이 없는지 iOS/Android에서 확인한다.
...
- 미완료: 실기기(iOS/Android) 탭/핀치 스모크 테스트
