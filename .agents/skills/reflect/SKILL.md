...
- `failed -> recovered` 또는 corrected rerun으로 통과한 검증은 Q4 실패표에서 제거하지 않는다. 원래 실패 명령, failure_class(`test_fixture_stale`/`environment_failure` 등), recovery action, recovered command를 Q4 근거로 남기고, `blocks_archive=false`인 경우에만 warning-success로 분류한다.
