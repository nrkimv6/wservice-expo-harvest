...
2. **TODO Pending + Plan 문서** - 동등한 우선순위로 모두 스캔
   ...
   - `{project}`의 resolved plan root 아래 `docs/plan/*.md` 미완료 항목 (프로젝트별 계획)
...
- wtools 감지 (현재 디렉토리에 `common/` 폴더 존재 여부):
  - **있으면**: `.worktrees/plans` 실경로가 있으면 그 plan 경로를 먼저 스캔하고, 없으면 AGENTS.md 문서 위치 규칙의 plan 경로를 스캔 (공통 계획)
  - **없으면**: AGENTS.md 문서 위치 규칙의 plan 경로만 스캔 (기본: `docs/plan/`)
- projects.json의 각 `{proj.path}`에서 resolved plan root 기준 `docs/plan/*.md` 파일들 스캔 (모든 15개 프로젝트)
- `[ ]` 또는 `[→TODO]` 상태인 항목 찾기
...
2. **파일 수정**: 선택한 항목의 `[ ]` → `[→$workerId]`로 변경
   - plan 파일: 방금 읽은 동일 실경로(plan root 또는 `.worktrees/plans`)에만 기록
