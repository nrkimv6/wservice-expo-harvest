...
**프로젝트가 정해진 경우:** AGENTS.md/CLAUDE.md 문서 위치 규칙의 plan 경로 아래 `YYYY-MM-DD_{주제}.md`
**plans worktree가 있으면:** 위 경로는 `Get-PlanRoot()`가 가리키는 cwd를 기준으로 해석
...
**모든 파일은 해석된 plan root 아래 plan 경로에 유지한다** — 즉시 archive 하지 않음. `/done`에서 archive.
...
**프로젝트가 정해진 경우:** AGENTS.md/CLAUDE.md 문서 위치 규칙의 plan 경로에 생성
**plans worktree가 있으면:** 위 경로는 `Get-PlanRoot()`가 가리키는 cwd를 기준으로 해석
**공통/복수 프로젝트:** plan은 AGENTS.md/CLAUDE.md 문서 위치 규칙의 plan 경로에, **TODO는 각 프로젝트별로 분리 생성**
