...
- {고려사항}
- wtools authoring surface 변경 plan은 one surface per executable TODO 원칙을 따른다. 여러 engine surface를 다루면 parent는 coordination-only로 두고, 실행 체크박스는 surface별 child `_todo-N.md`에만 둔다.
...
### Phase M: Merge Handoff (/merge-test owner, T4/T5 또는 live 검증 필요 시)

M. ☐ **main merge 이후 검증으로 넘긴다** — live-test phase fence
...
Python 수정 포함 시 구현 Phase 뒤에 **T1~T5 테스트 Phase 필수**. T3는 pre-merge local integration only이며 Browser MCP/localhost/restart/live API/http_live 검증은 금지한다.
live/browser/localhost 검증이 필요하면 `Phase M: Merge Handoff (/merge-test owner)` 이후 Phase T4 또는 Phase T5로 배치한다.
