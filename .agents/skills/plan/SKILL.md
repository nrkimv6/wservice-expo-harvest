...
**경로 규칙**: AGENTS.md `문서 위치 규칙` 테이블을 참조하라. 테이블이 없으면 기본 경로(`docs/plan/`, `docs/archive/`)를 사용한다. plans 워크트리가 있으면 `Get-PlanRoot()`/`Resolve-DocsCommitRoot()`가 가리키는 cwd 아래 문서 경로를 우선 사용한다. 상세: [`_path-rules.md`](./_path-rules.md)
...
**plans worktree 우선 규칙:**
- `.worktrees/plans/docs/plan`이 있으면 새 plan과 archive/TODO/DONE 수정은 모두 해당 worktree cwd에서 수행한다.
- plans worktree가 없으면 일단 repo root의 문서 경로를 사용하되, 최초 생성 전에 아래 bootstrap 절차를 먼저 시도한다.
- `wtools/TODO.md` 동기화도 문서 커밋 cwd와 동일한 plan root 기준으로 수행한다.

**plans worktree bootstrap (최초 1회 / 복구 공통):**
1. `origin/plans`가 있으면: `git worktree add .worktrees/plans plans`
2. `origin/plans`가 없으면:
   - `git worktree add .worktrees/plans -b plans`
   - `.worktrees/plans` cwd에서 초기 문서 커밋 후 `git push -u origin plans`
3. bootstrap 실패 시:
   - plans worktree가 필수인 흐름(문서 격리 강제 프로젝트)은 plan 생성 없이 즉시 중단
   - 사용자가 수동 복구해야 하는 실패 상태(브랜치 충돌, worktree 경로 충돌, push 실패)를 그대로 보고
   - root/main과 `.worktrees/plans`를 동시에 부분 수정한 채 진행하지 않는다
...
1. **plan 파일 존재** — `Get-PlanRoot()`가 가리키는 cwd의 plan 경로에 대표 문서 존재
...
2. 화이트리스트 파일만 **개별** `git add` — `Resolve-DocsCommitRoot` cwd에서 `Resolve-DocsCommitCandidates` 반환값만 add
   - 예: `git add "{plan경로}/YYYY-MM-DD_foo.md"`
   - 예: `git add "TODO.md"`
...
- plans 워크트리에서는 `Resolve-DocsCommitRoot`가 가리키는 cwd 기준 plan/archive/TODO/DONE 경로만 허용
...
**올바른 add 방법 (파일명 명시):**
- `git add "{plan경로}/2026-03-05_foo.md"`
- `git add "TODO.md"`
...
[단일]
plan: {plan경로}/YYYY-MM-DD_{주제}.md (분석 + TODO 포함, N phases, M tasks)

[분리]
plan: {plan경로}/YYYY-MM-DD_{주제}.md (대표 문서)
todo-1: {plan경로}/YYYY-MM-DD_{주제}_todo-1.md (Phase 1~3, N tasks)
todo-2: {plan경로}/YYYY-MM-DD_{주제}_todo-2.md (Phase 4~6, M tasks)
