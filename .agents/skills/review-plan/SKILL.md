...
**C. 기존 plan 중복 체크:**
- 문서 위치 규칙(AGENTS.md/CLAUDE.md)의 plan 경로 중 **실제 plan root가 가리키는 실경로**에 있는 기존 계획서 제목과 비교
...
- 화이트리스트 파일만 스테이징: `Resolve-DocsCommitRoot` 기준 cwd에서 `Resolve-DocsCommitCandidates`가 반환한 plan 문서와 `TODO.md`
